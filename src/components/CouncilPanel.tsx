              ...
              return (
                <div key={crop} className="flex items-center gap-3 my-2">
                  ...
                  <input
                    ...
                    step={1}
                    value={pct}
                    onChange={e => setLandSplit(crop, Number(e.target.value))}
          ...
          <div>
            ...
            <div className="flex items-center gap-3 mt-1">
              <input
                ...
                onChange={e => setField('yieldVariability', Number(e.target.value))}
                className="quill flex-1"
              />
      ...
      <Card>
        ...
        <div className="grid grid-cols-2 gap-3">
          <NumberField label="Woodland (ac)" tooltip="Acres of common woodland where peasants gather wood, peat, or turf."
            value={params.woodlandAcres} onChange={v => setField('woodlandAcres', v)} />
          <NumberField label="Yield/ac/yr" tooltip="Cartloads gathered per acre per year."
            step={0.1} value={params.fuelYieldPerAcre} onChange={v => setField('fuelYieldPerAcre', v)} />
          <NumberField label="Summer need" tooltip="Cartloads per household per month, for cooking only."
            step={0.1} value={params.fuelNeedsSummer} onChange={v => setField('fuelNeedsSummer', v)} />
          <NumberField label="Winter need" tooltip="Cartloads per household per month for cooking + heating."
            step={0.1} value={params.fuelNeedsWinter} onChange={v => setField('fuelNeedsWinter', v)} />
        </div>
      </Card>
        ...
        {showAdvanced && (
          <div className="px-5 pb-5 pt-2 space-y-5 border-t border-[rgba(120,80,30,0.18)]">
            <Subsection icon={<Sprout className="w-4 h-4" />} title="Base Yields">
              <div className="grid grid-cols-2 gap-3">
                <NumberField label="Wheat (bu/ac)" value={params.yields.wheat} onChange={v => setNested('yields', 'wheat', v)} />
                <NumberField label="Barley (bu/ac)" value={params.yields.barley} onChange={v => setNested('yields', 'barley', v)} />
                <NumberField label="Oats (bu/ac)" value={params.yields.oats} onChange={v => setNested('yields', 'oats', v)} />
                <NumberField step={0.1} label="Hay (tons/ac)" value={params.yields.hay} onChange={v => setNested('yields', 'hay', v)} />
              </div>
            </Subsection>

            <Subsection icon={<Flame className="w-4 h-4" />} title="Spoilage & Tithes">
              <div className="grid grid-cols-2 gap-3">
                <NumberField step={0.1} label="Grain spoil %/mo" value={params.spoilageRate} onChange={v => setField('spoilageRate', v)} />
                <NumberField step={0.1} label="Hay spoil %/mo" value={params.haySpoilageRate} onChange={v => setField('haySpoilageRate', v)} />
                <NumberField label="Tithe & mfg %" value={params.titheAndManufacturePct} onChange={v => setField('titheAndManufacturePct', v)} />
                <NumberField step={0.1} label="Wool lbs/sheep/yr" value={params.woolPerSheep} onChange={v => setField('woolPerSheep', v)} />
              </div>
            </Subsection>

            <Subsection icon={<Users className="w-4 h-4" />} title="Household Composition">
              <div className="grid grid-cols-3 gap-3">
                <NumberField step={0.1} label="Males" value={params.peoplePerHH.male} onChange={v => setNested('peoplePerHH', 'male', v)} />
                <NumberField step={0.1} label="Females" value={params.peoplePerHH.female} onChange={v => setNested('peoplePerHH', 'female', v)} />
                <NumberField step={0.1} label="Children" value={params.peoplePerHH.child} onChange={v => setNested('peoplePerHH', 'child', v)} />
              </div>
            </Subsection>
          </div>
        )}
...
function NumberField({ label, value, onChange, step, tooltip }: { label: string; value: number; onChange: (v: number) => void; step?: number; tooltip?: string }) {
  return (
    <div>
